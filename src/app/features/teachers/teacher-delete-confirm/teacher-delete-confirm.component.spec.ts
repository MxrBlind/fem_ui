...
} from './teacher-delete-confirm.component';

function makeRow(overrides: Partial<TeacherRow> = {}): TeacherRow {
  return {
    id: 42,
    name: 'Juan',
    ...
    phone: '5551234567',
    church: 'Central',
    raw: {} as TeacherRow['raw'],
    ...overrides,
  };
}

async function render(row: TeacherRow): Promise<{
  fixture: ComponentFixture<TeacherDeleteConfirmComponent>;
  component: TeacherDeleteConfirmComponent;
  ...
  const fixture = TestBed.createComponent(TeacherDeleteConfirmComponent);
  const component = fixture.componentInstance;
  fixture.detectChanges();
  return { fixture, component, dialogRef };
}

describe('TeacherDeleteConfirmComponent', () => {
  ...
  it('omits em-dash segments from the summary', async () => {
    const { component } = await render(
      makeRow({
        name: TEACHER_FALLBACK,
        parentLastName: TEACHER_FALLBACK,
        email: TEACHER_FALLBACK,
      })
    );
  ...
  it('keeps partial name when only one of the name fields is missing', async () => {
    const { component } = await render(
      makeRow({ parentLastName: TEACHER_FALLBACK })
    );
