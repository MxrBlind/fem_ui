...
export class SubjectEditComponent implements OnInit {
  ...
  private readonly dialogRef = inject(MatDialogRef<SubjectEditComponent, SubjectDto>);
  private readonly data = inject<SubjectEditDialogData>(MAT_DIALOG_DATA);
  ...
  readonly form = this.fb.nonNullable.group({
    code: new FormControl<string>(this.data.subject.code ?? '', {
      nonNullable: true,
      validators: [Validators.required, Validators.maxLength(50)],
    }),
    description: new FormControl<string>(this.data.subject.description ?? '', {
      nonNullable: true,
      validators: [Validators.required, Validators.maxLength(100)],
    }),
    categoryId: new FormControl<number | null>(this.data.subject.category?.id ?? null, {
      validators: [Validators.required],
    }),
    levelId: new FormControl<number | null>(this.data.subject.level?.id ?? null, {
      validators: [Validators.required],
    }),
  });
    ...
    forkJoin({
      ...
      .pipe(
        catchError((err: unknown) => {
          console.error('[subject-edit] failed to load form data', err);
      ...
      .subscribe((result: LoadResult | null) => {
        ...
        const currentCategoryId = this.data.subject.category?.id ?? null;
        if (currentCategoryId != null) {
          const found = result.categories.find((c) => c.id === currentCategoryId);
          if (found) {
            this.categorySearch.set(this.displayCategory(found));
          } else {
            this.form.controls.categoryId.setValue(null);
          }
        }

        const currentLevelId = this.data.subject.level?.id ?? null;
        if (currentLevelId != null) {
          const found = result.levels.find((l) => l.id === currentLevelId);
          if (found) {
            this.levelSearch.set(this.displayLevel(found));
          } else {
  ...
  onSubmit(): void {
    ...
    this.subjectService
      ...
      .subscribe({
        ...
        error: (err: unknown) => {
          console.error('[subject-edit] failed to update subject', err);
