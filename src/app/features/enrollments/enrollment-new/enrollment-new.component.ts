...
@Component({
  ...
  styleUrl: './enrollment-new.component.scss',
})
export class EnrollmentNewComponent implements OnInit {
  ...
  private readonly dialogRef = inject(MatDialogRef<EnrollmentNewComponent, EnrollmentDto>);
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    scholarshipPercent: new FormControl<number>(0, {
      nonNullable: true,
      validators: [
        Validators.required,
        Validators.min(0),
    ...
    forkJoin({
      ...
      .pipe(
        ...
        catchError((err: unknown) => {
          console.error('[enrollment-new] failed to load form data', err);
  ...
  onStudentInput(value: string): void {
    ...
    // If the user edits the text after selection, clear the stored id until
    // an option is re-selected so the form does not silently keep a stale id.
  ...
  onSubmit(): void {
    ...
    const { studentId, courseId, scholarshipPercent } = this.form.getRawValue();
    ...
    this.enrollmentService
      ...
      .subscribe({
        ...
        error: (err: unknown) => {
          console.error('[enrollment-new] failed to create enrollment', err);
