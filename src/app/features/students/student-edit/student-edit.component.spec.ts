...
interface Harness {
  fixture: ComponentFixture<StudentEditComponent>;
...
describe('StudentEditComponent', () => {
  ...
  describe('constants', () => {
    it('exports the exact success and error messages', () => {
      expect(SUCCESS_MESSAGE).toBe('Registro actualizado exitosamente');
      expect(ERROR_MESSAGE).toBe('Error al actualizar este registro');
    });
  });
  ...
  describe('prefill', () => {
    ...
    it('renders the dialog title "Actualizar estudiante"', () => {
      const { fixture } = setup();
      const title = (fixture.nativeElement as HTMLElement).querySelector(
        'h2[mat-dialog-title], h2.mat-mdc-dialog-title'
      );
      expect((title?.textContent ?? '').trim()).toBe('Actualizar estudiante');
    });
  });
  ...
  describe('submit', () => {
    it('PUTs the exact payload with role and without password when blank', () => {
      ...
      expect(req.request.body.role).toEqual({ id: 3 });
    ...
    it('includes password when the user types one and keeps role in payload', () => {
    ...
    it('serializes birthDate to the ISO format required by the backend', () => {
      const { fixture, http, user } = setup();
      fixture.componentInstance.form.controls.birthDate.setValue(
        new Date(1969, 4, 31)
      );
      fixture.componentInstance.onSubmit();

      const req = http.expectOne(USER_URL(user.id));
      expect(req.request.body.profile.birthDate).toBe(
        '1969-05-31T00:00:00.000+00:00'
      );
      req.flush(user);
    });

    it('shows error snackbar, keeps dialog open, re-enables form (username stays disabled), and logs on failure', () => {
      const errSpy = vi
        .spyOn(console, 'error')
